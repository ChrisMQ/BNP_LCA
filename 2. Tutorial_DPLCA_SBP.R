...
# Function used to simulate mixture of binary and continuous variables
...
bcode <- nimbleCode({
  ...
  for(t in 1:H) {
    for(j in 1:J) {
      ip[t, j] ~ dbeta(shape1 = a1, shape2 = a2)
    }
  }
  # stick-breaking process
  alpha ~ dgamma(shape = 2, rate = 2)
  for(t in 1:(H-1)) { # stick-breaking variables
    v[t] ~ dbeta(1, alpha)
  }
  w[1:H] <- stick_breaking(v[1:(H-1)]) # stick-breaking weights
})

# Run the MCMC
nMCMC <- 10000
consts <- list(N = N, J = J, H = 10, a1 = 1, a2 = 1)
set.seed(123)
inits <- list(ip = matrix(rbeta(N*J, shape1 = 1, shape2 = 1), nrow = N, ncol = J),
              v = rbeta(consts$H-1, 1, 1), alpha = 1,
              z = sample(1:5, size = consts$N, replace = TRUE))
dat.b <- list(y = as.matrix(simdat$dat))
model <- nimbleModel(bcode, data = dat.b, inits = inits, constants = consts,
                     dimensions = list(ip = c(N, J))) 
cmodel <- compileNimble(model) 
conf <- configureMCMC(model, monitors = c('z','w','alpha','ip'), print = TRUE) 
mcmc <- buildMCMC(conf) 
...
wSamples <- samples.b[, grep('w', colnames(samples.b))]
...
w.post <- wSamples[nburn+seq(1, (nMCMC-nburn), nthin), ] 
...
N <- 500 # sample size
P <- c(0.5, 0.3, 0.2) # class weights 
# 3 components, 5 polytomous variables with different number of categories
...
pcode <- nimbleCode({
  ...
  for(t in 1:H) {
    for(j in 1:J) {
      ip[t, j, 1:C[j]] ~ ddirch(beta[1:C[j]])
    }
  }
  # stick-breaking process
  alpha ~ dgamma(shape = 2, rate = 2)
...
# Run the MCMC
nMCMC <- 10000
constants <- list(N = N, J = J, H = 30, C = C, beta = rep(1, C.max))
set.seed(123)
...
ccode <- nimbleCode({
  ...
  for(t in 1:H) {
    for(j in 1:J) {
      ...
      mu[t, j] ~ dnorm(mean = nu1, tau = nu2)
      tau[t, j] ~ dgamma(a1, a2) # tau = 1/variance
    }
  }
  # stick-breaking process
  alpha ~ dgamma(shape = 2, rate = 2)
...
# Run the MCMC
nMCMC <- 10000
constants <- list(N = N, J = J, H = 30, a1 = 1, a2 = 1, nu1 = 0, nu2 = 1)
set.seed(123) 
mutau.int <- rnormgamma(J*N, 0,1, 1,1) # generate initial values for mu and tau
inits <- list(z = sample(1:5, size = N, replace = TRUE), 
              v = rbeta(consts$H-1, 1, 1), alpha = 1,
...
# Simulate a dataset
set.seed(123)
simdat <- mixdat.sim(J, L, K, N, props = P, ips = ip, mus, vars)
...
mixcode <- nimbleCode ({
  ...
  for(t in 1:H) {
    ...
    for (l in 1:L) {
      # set independent priors to item mean and precision
      mu[t, l] ~ dnorm(nu1, tau = nu2)
      tau[t, l] ~ dgamma(b1, b2) 
    }
  }
  # stick-breaking process
  alpha ~ dgamma(shape = 2, rate = 2)
...
# Run the MCMC
nMCMC <- 10000
consts <- list(N = N, J = J, L = L, H = 30, a1 = 1, a2 = 1, b1 = 1, b2 = 1, nu1 = 0, nu2 = 1)
set.seed(123)
